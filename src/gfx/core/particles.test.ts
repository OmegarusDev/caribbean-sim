...
describe('ParticlePool', () => {
  it('spawns and updates without allocation churn', () => {
    ...
    pool.spawn(1, 2, 10, 20, 0.5, 0.5, 4, 'flash');
    pool.spawn(3, 4, 0, 0, 0.1, 0.5, 2, 'smoke');
    ...
    expect(pool.data[base + 4]!).toBeCloseTo(0.1);
  });

  it('compacts to the front after deaths', () => {
    const pool = new ParticlePool(8);
    pool.spawn(1, 1, 0, 0, 0.2, 1, 2, 'flash');
    pool.spawn(2, 2, 0, 0, 0.9, 1, 2, 'flash');
    pool.spawn(3, 3, 0, 0, 0.3, 1, 2, 'flash');
  ...
  it('respects capacity', () => {
    ...
    for (let i = 0; i < 10; i++) {
      pool.spawn(i, 0, 0, 0, 1, 1, 1, 'flash');
    }
  ...
  it('kills particles by life and keeps the pool usable', () => {
    const pool = new ParticlePool(8);
    for (let i = 0; i < 6; i++) pool.spawn(0, 0, 0, 0, 0.05, 1, 1, 'flash');
    for (let i = 0; i < 40; i++) pool.update(0.1);
    expect(pool.count).toBe(0);
    pool.spawn(1, 1, 0, 0, 1, 1, 1, 'ring');
    expect(pool.count).toBe(1);
  });
});
